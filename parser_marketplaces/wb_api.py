...
class WildberriesAPI:
    ...
    """
    'ab_testing': включение отлюкчение тестирования
    'appType': с какой платформы ты сидишь(веб, телефон и т д)
    'curr': 'rub' в какой валюте выводить цены
    'dest': '-364776' id пункта выдачи вб
    'hide_dtype': '13' скрывает какие то определённые типы данных из поиска
    'lang': 'ru' Язык интерфейса
    'page': '1' номер страницы
    'query': query наш запрос товара
    'resultset': 'catalog' Описание: Тип данных, которые нужно получить. 
     В данном случае — каталог товаров или услуг.
    'sort': 'popular', параметр сортировки
    'spp': '30', количество результатов на одной странице
    'suppressSpellcheck': 'false'
    Описание: Если 'false', то проверка правописания (например, исправление опечаток в запросе) включена. 
    Если 'true', то проверка отключена.
    'uclusters': '3', Описание: Возможно, этот параметр связан с группировкой результатов. Например, '3' может означать, 
    что результаты будут разделены на три группы (кластера).
    'uiv': '0' Описание: Этот параметр может быть связан с версией пользовательского интерфейса (UI Version)
    """
    ...
    def save_images(product_id, product_pics, save_image_all, timeout=10):
        ...
        for i in range(1, product_pics + 1):
            ...
            try:
                ...
                print(f"Сервер вернул ошибку: {err.response.status_code}")
